...
- **受保护文档归位（审计 GAP-1 结项）**：8 个受保护文档此前被误跟踪（`ChangeRequest-*.md`、`DesignSpec-*-执行方案.md` ×3、`ui-design-*.html` ×2、`deploy/aistudy-run-port.sh`、`小白AI课程8周学习路径.md`）→ `git rm --cached` 移出索引（**文件与内容全部保留在盘上**，历史提交仍可回溯）+ `.gitignore` 显式忽略，从此 `git add -A` 也不可能把它们加回索引（机械保护，不靠人记纪律）。**不改历史、不 `--force`**。
