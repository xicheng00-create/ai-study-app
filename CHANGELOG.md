...
### Added
- **教师端测评草稿预览（QUIZ-001 P0 补全）**：生成草稿后，草稿卡片新增「👁 预览」按钮，点开用底部弹层展示该草稿全部题目（题型/分值/选项/参考答案），教师在确认发布前可审核题目内容与答案是否正确；预览弹层内可直接「确认发布」或「关闭」。补齐设计规格 QUIZ-001「生成草稿→预览微调→确认发布」中缺失的「预览」环节——此前草稿卡只有「确认发布/放弃」，无法点开查看题目（后端 `GET /api/quizzes/:id` 本就返回草稿题目+答案，仅前端缺预览入口）。
