...
def test_choice_deterministic():
    ...
    assert grader.grade_question(q, "1")["score"] == 5.0
    assert grader.grade_question(q, "0")["score"] == 0.0


def test_bool_deterministic():
    q = _q("bool", "正确")
    assert grader.grade_question(q, "正确")["correct"] == 1
    assert grader.grade_question(q, "正确")["score"] == 5.0
    assert grader.grade_question(q, "错误")["correct"] == 0
    assert grader.grade_question(q, "错误")["score"] == 0.0


def test_essay_three_tiers():
    # 参考答案关键词：反向传播、链式法则、梯度（问答满分 10）
