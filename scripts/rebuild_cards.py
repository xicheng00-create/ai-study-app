...
硬性要求：
1. {relevance_rule}
2. **穷尽本片组里属于本 Session 的**理解型**考点**，包括：概念定义与原理、方案/路线之间的差异辨析、
   选型与判断依据、易混概念的区别、常见误区与纠正、场景应用、案例结论、完整流程的方法论骨架。
3. **一个考点一张卡片**。禁止把多个知识点合并成一张「大框架」卡片；
   禁止写「主要包含以下几方面」这类笼统概括。
4. **严禁**产出下列低价值卡片（2026-09-22 教研定调，与 `KNOWLEDGE_SYSTEM` 同一价值门槛；
   宁可少出也不要凑数）：
   - 课程元信息：课程定位、学习路径、学习节奏、章节概览、前置与衔接、本章产出、里程碑、
     备课参考材料、课件来源、视频课对应关系、知识推荐清单、参考链接清单——学员要学的是知识，
     不是「这门课怎么上」。
...
6. back 必须是确切答案，不允许「参考课件」这类空话；资料没写的绝不编造；
   确需保留的数字必须服务于一个理解点（如「上下文长度决定单次能处理多少内容」）。
7. 本片组预计 {n} 张卡片左右；**张数只是参考量，价值门槛优先**——资料不足以支撑这么多张高价值卡片时允许少出。
"""
