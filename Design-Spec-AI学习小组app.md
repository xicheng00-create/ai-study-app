...
- **落地方式（两层，可复现）**：
  ...
  - ④ **受保护文档处理**：`小白AI课程16章学习路径.md`（原 `小白AI课程8周学习路径.md`）是课件引用 30 次的「课程总路径」源文档，同时在**受保护文档清单**里 —— 按 CLAUDE.md §6.7「冲突不硬解」，先报 Hermes 由用户裁决；用户选「先归位 GAP-1（移出跟踪）再改名」，于是：`git rm --cached` 8 个受保护文档 + `.gitignore` 显式忽略（**机械保护**）→ 再改名 + 正文迁移 + 同步 30 处引用。
- **验证**：`make lint test smoke` 全绿；口径残留自检 **卡片 5 处（均领域语义）/ 教案 md 0 处 / md 切片 0 处 / 课程总路径文档 0 处**；课件目录 16 个改名后文件计数 939 不变；`materials.source_path` 全库 `课件/W%` 命中 **0**。
- **教训（新增，防再犯）**：
  ...
  4. **受保护文档冲突先报再动**：本次是「先归位跟踪状态 → 再改内容」的正确顺序；若先改名，`git` 会把它记成「删除 + 新增未跟踪」，反而污染索引。
