...
| KNOW-008 | student | P1 | **浏览卡片必须按主题分组（v2.7.0，CR-2026-0922-CARDSGROUP）**：一章 300~650 张卡平铺不可用（用户实报「每个 session 几百张卡片挺多的」）。① 浏览页（「知识卡片」列表）按「**章 → 主题组 → 卡片**」两级钻取：**纵向折叠、点组展开、默认全折叠、无横向滚动**；② 主题组 = 该章卡片的 LLM 归类结果（每组 **5~80 张**，目标 15~80，**禁止**出现吞掉全章一半的巨型组、也禁止 1~4 张的碎片组；碎组并入最贴切的大主题，实在无处可去才落「其他要点」）。**清退例外（对后续一律适用，Ray 2026-09-26 裁决）**：该 5~80 张约束**只在分组生成时点生效**；已发布组的卡片被后续清理（低价值清退 / 去重合并）删减到 1~4 张时**允许存留、不触发重跑**，等下一次对该章重新分组时自然收敛。**触发点** = `scripts/group_cards.py` 的再平衡工序（`MIN_TOPIC_CARDS=5`）只在**生成/重跑**时执行，不对存量组做巡检；**覆盖路径** = 浏览页 `GET /api/knowledge/<chapter_id>` 对 1~4 张组照常下发 `topic`，前端不得因组小而不渲染、报错或自动并组（前端无并组逻辑）；**违规处置** = 若有人为「凑够 5 张」而回填已清退卡片、或在未走变更单的情况下重跑**已发布章**分组，视为越界（重跑须走变更单 + bump 版本 + 回写本条款）；③ 归类**只做浏览标签**，**不得**改卡片正文、切片绑定、复习态，也不参与出卡/复习调度（分组失败或未归类时 `topic` 返回空串 → 前端退化为单组「全部卡片」，老库同样可用）；④ 分组结果落库 `card_topics` 并随 `GET /api/knowledge/<chapter_id>` 的卡片字段 `topic` 下发；⑤ 组内统计（张数/已掌握/学习中/未学/今日待复习）与章级一致 |
...
- **卡片主题分组（v2.7.0，CR-2026-0922-CARDSGROUP，KNOW-008）**：`scripts/group_cards.py [--chapter N] [--apply]`——LLM 三级处理：① 主题表（资料名 + 高频 `sub_concept` + 抽样 front → 10~16 个主题，要求体量均衡）；② 逐卡归类（每批 25 张，只允许用给定主题）；③ **再平衡**（首轮实测模型会造出「模型选型与对比(280)」式巨型桶 + 一堆 1~4 张碎片组，故加大于 `MAX_TOPIC_CARDS=80` 自动拆 2~4 子主题、小于 `MIN_TOPIC_CARDS=5` 并入大主题两道工序，最多拆 2 轮）。**注意**：5~80 张只在生成/重跑时生效，清退后存量的 1~4 张组允许存留（见 §3.4 KNOW-008 ②「清退例外」条款）。落库 `card_topics`（card_id PK, chapter_id, topic, ord），按章先删后插幂等；默认 dry-run，`--apply` 才写库且**只写 card_topics**。建表复用 `backend/data/models.py` 的 SCHEMA/migrate（单一真相，不重复 DDL）。`topic` 经 `LEFT JOIN card_topics` 随卡片下发，无归类时为空串（前端退化为单组「全部卡片」）。
...
- **待人工裁决 / 观察项（本轮未动数据、未改口径）**：2026-09-24 清退（-621 张）后，已发布章 `card_topics` 86 组中现有 **6 组 <5 张**（最小「闭源旗舰模型对比」1 张）。KNOW-008 的 5~80 张约束作用于**分组生成时点**，卡库瘦身后组内卡数自然下降。是否重跑 `scripts/group_cards.py` 再平衡，或把该条款补一句「清理后允许存留 <5 张组，下次分组时收敛」，**交 Ray 裁决**。→ **2026-09-26 已决：Ray 裁决「补条款」，条款正文见 §3.4 KNOW-008 ②「清退例外」与 §12.54。**
...
### 12.53 实现状态回写（v2.9.5，2026-09-26，NOTIF-012 移除「今晚不再提示」出口）

> 触发：Ray 2026-09-26「hermesstu 不小心点了今晚不再提醒按钮，请撤回，然后删掉这个按钮」。

- **口径（对后续一律适用，写进 §3.12 NOTIF-012 正文）**：催学类弹窗/通知**一律不得提供「不再提示／稍后／静音」类出口**。理由：该出口与「不择手段催学」目标直接冲突，一次误点即当天失去唯一不依赖推送授权的催学手段。**违规处置**：日后若有人在催学弹窗/催学通知里新增此类出口（含「关闭后今天不再提醒」「稍后提醒」等变体），视为需求缺陷，须删除并 bump 版本回写本条款。
- **变更**：`js/app.js` 弹窗只保留「立即去做」；`js/student.js` 删除 `doNagLater()`；`js/app.js` 删除 `aistudy_nodisturb_<date>` 频控读取。每设备每天 1 次的 `aistudy_nag_<date>` 频控**保留**（防反复打扰，与「静音出口」性质不同）。
- **关于「撤回」的实测取证（重要，避免误判）**：该按钮**只写设备本地 localStorage**（`aistudy_nodisturb_<YYYY-MM-DD>`），**不落库、不经接口**，因此「撤回」无服务端状态可改：① 键按本地日期命名，跨零点自动失效；② 服务端阶梯提醒（19:00–23:00 Web Push）**完全不受它影响**。实测生产库 `notification_prefs`：`hermesstu`（`c1e2aabc-…`）= `push_enabled=1, remind_daily=1`，**提醒开关一直是开着的**。故本次无需（也无法）回滚数据，只做代码层删除。
- **版本三件套**：`app.py` `version = "2.9.5"` / `sw.js` `CACHE = "aistudy-shell-v61"` + `V = "2.9.5"` / `index.html` 6 处 `?v=2.9.5`。CHANGELOG 顶条 `## [2.9.5] - 2026-09-26`。
...
### 12.54 实现状态回写（文档口径补充，2026-09-26，KNOW-008 ②「清退例外」条款）

> 触发：Ray 2026-09-26 一句「补条款」——关闭 §12.52 遗留的待人工裁决项。**纯文档改动**：无运行时代码变更，`backend/app.py` version 保持 `2.9.5`（health 不变属预期，无需重启）。

- **裁决**：**不重跑** `scripts/group_cards.py`；改为在 KNOW-008 ② 补「**清退例外**」条款（对后续一律适用），并同步标注技术小节与 §12.52 / §12.53 的历史条目状态（**就地标注，不重写原记录**）。
- **条款要点**：5~80 张只约束**分组生成时点**；存量组因后续清退（低价值清退 / 去重合并）掉到 1~4 张 → 允许存留，下次对该章重新分组时自然收敛。**触发点 / 覆盖路径 / 违规处置**三要素已写进条款正文（§3.4）。
