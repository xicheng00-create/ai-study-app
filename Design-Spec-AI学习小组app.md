...
- **NOTIF-010（P0）提醒可达性：学生端常显引导 + 教师端可见订阅状态**：学生端「学习」首页**常显**「提醒」卡片（不得按状态隐藏）：未开启 → 「🔔 开启提醒，别断连胜」+ iPhone 指引（英文 label）+「立即开启」按钮；已开启 → 「✓ 提醒已开启 · 每晚 19:00–23:00 未达标最多提醒 5 次」。教师端「班级活动」页新增「今日打卡 · 提醒可达性」卡片：每人一行 `已打卡 / 未打卡` + `🔔 已开提醒 / 未开提醒（红色）`。状态判据 = **订阅存在性**（`push_subscriptions`），不是 `push_enabled` 布尔值。 ✅
...
| NOTIF-010 | ✅ | 学生端常显提醒卡 + 教师端 checkin-board 订阅状态；判据 = 订阅存在性 |
